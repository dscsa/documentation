...
from
    "datawarehouse".dev_analytics."quickbook_general_ledger" qgl
left join (select * from "datawarehouse".dev_analytics."quickbook_classes"
    where id in (
        select id
...
left join "datawarehouse".dev."quickbook_accounts_top_level" qa on (qa.id = qgl.account_id)
left join "datawarehouse".dev."quickbook_accounts_top_level" qap on (qap.id = qa.top_level_id)
left join (select * from "datawarehouse".dev_analytics."quickbook_customers"
    where id in (
        select id
        from "datawarehouse".dev_analytics."quickbook_customers"
        group by id
        having _airbyte_emitted_at = max(_airbyte_emitted_at)
