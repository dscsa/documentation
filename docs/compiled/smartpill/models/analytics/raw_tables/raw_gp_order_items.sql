...
with r as (
	select
		...
		_airbyte_ab_id,
		_airbyte_data,
		concat(cast(jsonb_extract_path_text(_airbyte_data, 'invoice_number') as int), '_', cast(jsonb_extract_path_text(_airbyte_data, 'rx_number') as int)) as pkey,
...
from
	...
	where greatest(created_at, updated_at, _ab_cdc_deleted_at) >= (select max(greatest(created_at, updated_at, date(_ab_cdc_deleted_at))) from "datawarehouse".dev_analytics."raw_gp_order_items")	
